...
impl Display for PrettyPrintableFloat {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        // Do the printing.
        let mut result = write!(f, "{}\n", ((pre_radix_point as f64) + post_radix_point)*(if is_negative { -1f64 } else { 1f64}));
        result = write!(f, "S/Exponent/Mantissa:\n");
        result = write!(f, "{}/{:8b}/{}", if is_negative { "1" } else { "0" }, exponent, mantissa);
        result
    }
}
...
fn main() {
    let pp = PrettyPrintableFloat::new(-9.4);
    println!("{}", pp);
}
